...
def load_model(model_path, num_classes=101):
    ...
    model = ResNetTransferModel(num_classes=num_classes, embedding_size=512, pretrained=False).to(device)
    try:
    ...
    if uploaded_file is not None or search_button:
        ...
        if search_button and uploaded_file:
            ...
            with st.spinner("Processing image..."):
                ...
                k = min(num_results, len(indexed_paths))
                similarities, indices = search_similar_images(query_feature, faiss_index, k=k)
